...
console.log('flow-test: click checkbox on engine.ts')
let staged = await stagedFiles()
check(staged.includes('util.js') && staged.includes('README.md'), `initially staged: ${JSON.stringify(staged)}`)
...
check(staged.includes('engine.ts'), `engine.ts staged after click (${JSON.stringify(staged)})`)

console.log('flow-test: click checkbox again to unstage')
await js(`[...document.querySelectorAll('.file-row')].find(r => r.querySelector('.file-name')?.textContent === 'engine.ts').querySelector('.file-checkbox').click()`)
await sleep(1500)
staged = await stagedFiles()
check(!staged.includes('engine.ts'), `engine.ts unstaged after second click (${JSON.stringify(staged)})`)
...
console.log('flow-test: commit one file')
await js(`[...document.querySelectorAll('.file-row')].find(r => r.querySelector('.file-name')?.textContent === 'app.ts').querySelector('.file-checkbox').click()`)
await sleep(1200)
await js(`(() => { const el = document.querySelector('.commit-summary'); const s = Object.getOwnPropertyDescriptor(window.HTMLTextAreaElement.prototype, 'value').set; s.call(el, 'Commit app.ts via GUI'); el.dispatchEvent(new Event('input', { bubbles: true })); })()`)
await sleep(300)
await js(`document.querySelector('.btn-commit').click()`)
await sleep(2000)
const msg = await headMsg()
check(msg === 'Commit app.ts via GUI', `HEAD message: "${msg}"`)
const files = await js(`document.querySelectorAll('.file-row').length`)
check(files === 4, `4 files remain after commit (${files})`)
